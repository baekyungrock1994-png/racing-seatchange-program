...
export class CircuitMaps {
  static getMap(themeId: CircuitThemeId): CircuitMapData {
    ...
    // 2. 결승선: 교실 남쪽 정문 게이트(col 11, row 9의 상단 경계 / y = 1800)
    const finishLine = {
      x: 11 * CELL_SIZE + 15,
    ...
    const isConnected = (idx: number, targetCol: number, targetRow: number) => {
      ...
      if (idx === trackCells.length - 1 && targetCol === 11 && targetRow === 8) {
        return true;
      }
      return false;
    };
   ...
   * 초대형 교실(1700px x 1600px)에 맞춰 자동차 4대가 여유롭게 지나갈 수 있는 통로가 보장됩니다.
   */
  private static getThemeLayout(themeId: CircuitThemeId, cellSize: number) {
    let cells: Array<{ col: number; row: number }> = [];
    // 초대형 교실 영역: 가로 1700px, 세로 1600px (자동차 4대 이상 통과 간격 완벽 보장)
    const classroomArea = {
      x: 9 * cellSize,
      y: 1 * cellSize,
      width: 8.5 * cellSize,
    ...
    if (themeId === 'classic') {
      ...
      cells = [
        ...
        { col: 1, row: 12 }, { col: 1, row: 11 }, { col: 1, row: 10 }, { col: 1, row: 9 }, { col: 1, row: 8 }, { col: 1, row: 7 }, { col: 1, row: 6 },
        ...
        // 5. 남쪽 루프
        { col: 8, row: 7 }, { col: 8, row: 8 }, { col: 8, row: 9 }, { col: 8, row: 10 }, { col: 8, row: 11 }, { col: 8, row: 12 },
        // 6. 최후의 가속 주로 (서->동)
        { col: 9, row: 12 }, { col: 10, row: 12 }, { col: 11, row: 12 },
        // 7. 결승선 진입 (남->북)
        { col: 11, row: 11 }, { col: 11, row: 10 }, { col: 11, row: 9 }
      ];
    } else if (themeId === 'classroom') {
      // ✏️ 2. 교실 어드벤처: 학용품과 책상 사이를 통과하는 직각 90도 크랭크 미로
      cells = [
        // 출발 및 1번 코너
        { col: 1, row: 12 }, { col: 1, row: 11 }, { col: 1, row: 10 }, { col: 1, row: 9 }, { col: 1, row: 8 }, { col: 1, row: 7 },
        // 연필 크랭크 1
        ...
        { col: 6, row: 9 }, { col: 6, row: 10 }, { col: 6, row: 11 }, { col: 6, row: 12 },
      ...
      cells = [
        ...
        // 4. 남쪽 대형 가속 링
        { col: 7, row: 7 }, { col: 7, row: 8 }, { col: 7, row: 9 }, { col: 7, row: 10 }, { col: 7, row: 11 }, { col: 7, row: 12 },
        { col: 8, row: 12 }, { col: 9, row: 12 }, { col: 10, row: 12 }, { col: 11, row: 12 },
        // 5. 결승 도킹 베이 진입
        { col: 11, row: 11 }, { col: 11, row: 10 }, { col: 11, row: 9 }
      ];
      ...
      cells = [
        ...
        // 4. 로터리 남측 출구로 탈출
        { col: 4, row: 6 }, { col: 4, row: 7 }, { col: 4, row: 8 }, { col: 4, row: 9 }, { col: 4, row: 10 },
        // 5. 운하 옆 그랜드 애비뉴 직선로
        { col: 5, row: 10 }, { col: 6, row: 10 }, { col: 7, row: 10 }, { col: 8, row: 10 }, { col: 9, row: 10 }, { col: 10, row: 10 },
        // 6. 광장 승리 개선문 -> 교실 결승선
        { col: 10, row: 11 }, { col: 11, row: 11 },
        { col: 11, row: 10 }, { col: 11, row: 9 }
      ];
    } else if (themeId === 'colosseum') {
      // ⚔️ 5. 고대 콜로세움: 투기장 원형 외곽 트랙과 2중 전차 헤어핀 시케인
      // 출발선(col 1)과 귀환로(col 3) 사이에 col 2의 200px 완전 버퍼 및 방호벽을 확보하여 숏컷 100% 차단!
      cells = [
        // 1. 투기장 서측 아치 트랙 (남->북)
        { col: 1, row: 12 }, { col: 1, row: 11 }, { col: 1, row: 10 }, { col: 1, row: 9 }, { col: 1, row: 8 }, { col: 1, row: 7 }, { col: 1, row: 6 },
        { col: 1, row: 5 }, { col: 1, row: 4 }, { col: 1, row: 3 }, { col: 1, row: 2 }, { col: 1, row: 1 },
        ...
        // 6. 승리의 전차 질주로 (col 3으로 내려가서 col 1 출발선과 200px 완전 격리)
        { col: 3, row: 9 }, { col: 3, row: 10 }, { col: 3, row: 11 }, { col: 3, row: 12 },
        { col: 4, row: 12 }, { col: 5, row: 12 }, { col: 6, row: 12 }, { col: 7, row: 12 }, { col: 8, row: 12 }, { col: 9, row: 12 }, { col: 10, row: 12 }, { col: 11, row: 12 },
        // 7. 콜로세움 정문 진입 (피니시)
        { col: 11, row: 11 }, { col: 11, row: 10 }, { col: 11, row: 9 }
      ];
    } else if (themeId === 'ice') {
      ...
      cells = [
        ...
        // 6. 설원 활강로 (남단)
        { col: 3, row: 8 }, { col: 3, row: 9 }, { col: 3, row: 10 }, { col: 3, row: 11 },
        { col: 4, row: 11 }, { col: 5, row: 11 }, { col: 6, row: 11 }, { col: 7, row: 11 }, { col: 8, row: 11 }, { col: 9, row: 11 }, { col: 10, row: 11 }, { col: 11, row: 11 },
        // 7. 얼음 궁전(교실) 진입 피니시
        { col: 11, row: 10 }, { col: 11, row: 9 }
      ];
    }
